  ...
  return (
    <div className="bg-[#050505]">
      <section className="max-w-6xl mx-auto px-4 py-20">
        <h1 className="text-4xl font-bold mb-10 text-white">作品集</h1>
          ...
          {projects.map((p, i) => (
            p.link ? (
              <Link
                key={i}
                to={p.link}
                className="group rounded-xl border border-white/10 bg-white/[0.03] overflow-hidden hover:border-white/20 hover:bg-white/[0.06] transition-all hover:-translate-y-1"
              >
                ...
                <div className="p-5">
                  <div className="flex flex-wrap gap-2 mb-3">
                    {p.tags.map(tag => (
                      <span
                        key={tag}
                        className="px-2.5 py-0.5 bg-white/10 text-white/70 rounded-full text-xs font-medium"
                      >
                        {tag}
                      </span>
                    ))}
